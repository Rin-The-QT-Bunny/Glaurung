...
def train_tasks(tasks,EPOCH,eta):
    ...
    for epoch in range(EPOCH):
        ...
        plt.plot(history)
        plt.pause(0.1)
        print("|Epoch: {} | Accuracy: {}| Average-LogP: {}|".format(epoch,r,math.exp(-logP)))
        if epoch%10 == 0:
            syns.save_model()
    plt.plot(history)
...
Tasks = [
    ...
    ["Turn around for","Turn(STOP())"],
    ["Turn around about","Turn(STOP())"],

    ["Open fire","Fire(STOP())"],
    ["exterminate","Fire(STOP())"],
    ["fire forward","Fire(STOP())"],
    ["exterminate the enemy","Fire(STOP())"],
    ["destroy the enemy","Fire(STOP())"],
    ["destroy them","Fire(STOP())"]
...
train_tasks(Tasks,526,0.000002)
syns.save_model()
plt.show()
